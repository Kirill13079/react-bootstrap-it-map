...
const Keys = (props) => {
  const { keys, selectedKey, vacancies, onCheckedKey, onSelectedKey } = props
    ...
    return props.keys.map((item, i) => (
      <div key={i} className="key__item--wrapper">
        <div
          ...
          onClick={(e) =>
            item.isChecked ? onSelectedKey(item.key) : alert('lox')
          }
        >
          <div className="key__item--img">
            <img
            ...
            <div className="key__item__content--info">
              <span>
                {vacancies
                  .get(keys[i].key)
                  .map((v, j) =>
                    props.vacancies.get(keys[i].key).length === j + 1
                      ? `${v.name} `
                      : `${v.name}, `
                  )}
              </span>
        ...
        <ListButtons>
          <Button
            onClickHandler={() => onCheckedKey(item.key)}
            bgColor="#fff"
            fill="#229fd9"
            icon={
              item.isChecked
